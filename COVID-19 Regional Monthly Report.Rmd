...
text_last_month <- custom_number_format(as.integer(new_avg_agg[match(nth(new_avg_agg$month, -2), new_avg_agg$month), 2]))

text_if_last_month_worst <- ifelse(new_avg_agg[match(nth(new_avg_agg$month, -2), new_avg_agg$month), 2] == max(new_avg_agg$avg_new_cases), "the worst month since the start of the pandemic.", "an improvement compared to the peak, in ")

text_worst_month <- ifelse(new_avg_agg[match(nth(new_avg_agg$month, -2), new_avg_agg$month), 2] == max(new_avg_agg$avg_new_cases), "", format(as.Date(as.integer(new_avg_agg[match(max(new_avg_agg$avg_new_cases), new_avg_agg$avg_new_cases), 1]), origin = "1970-01-01"), format = "%B %Y"))

text_worst_month_number <- ifelse(new_avg_agg[match(nth(new_avg_agg$month, -2), new_avg_agg$month), 2] == max(new_avg_agg$avg_new_cases), "", paste("(", custom_number_format(max(new_avg_agg$avg_new_cases)), "cases)"))
...
case_region_graph <- case_region %>% group_by(date) %>% summarize(new_cases = sum(new_cases)) %>% cbind(country = c("All"))

case_region_graph <- case_region %>% rbind(case_region_graph) %>% select(country, date, new_cases) %>% arrange(factor(country, levels = c("All")))
